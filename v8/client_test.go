...
func RedisIsAvailable(client redis.Cmdable) bool {
	return client.Ping(context.TODO()).Err() == nil
}

func RedisDeleteKeys(client redis.Cmdable) int64 {
	res, _ := client.Del(context.TODO(), "key1", "key2").Result()
	return res
}

func RedisMGetKeys(client redis.Cmdable) []interface{} {
	res, _ := client.MGet(context.TODO(), "key1", "key2").Result()
	return res
}

func RedisScan(client redis.Cmdable) []string {
	page, _, _ := client.Scan(context.TODO(), 0, "myredis.hash.*", 1000).Result()
	return page
}
